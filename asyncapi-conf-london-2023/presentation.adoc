...
== Our recipe
[.text-left]
Ask "politely" :) the teams to start their service from a GitHub template repo.

Service structure::
- `api/spec` - OpenAPI specification
- `api/external-specs` - AsyncAPI specifications, referenced from external repository
- `api/config` - code generation configuration, used by Modelina tools

[.notes]
--
It should be noted that this structure is NOT enforced in any way. It is just a convention that we are using. To help teams do the right thing we have a GitHub template repository that they can use to bootstrap their services.

It is worth noting that while initially we intended to use Modelina just for AsyncAPI and Kafka we quickly realized that we can do much, much more.

Like::
- Generate classes for REST layer
- Generate classes for Kafka communication
- Generate classes from JSON schema for storing data into CosmosDB or MongoDB
--
